...
## to proceed
## if room has no unexplored paths
## run bfs to find one

def pathfinder(bfs_path):
    ## interpret path output
    ## into directions and add
    ## to traversal list
    dir_list = []

    for room in bfs_path:
        print(f"room {room} in pathfinder on bfs_path {bfs_path}")
        for dir in visited[bfs_path[0]]: # maybe pop this instead
            ## check for match and add
            if room == visited[bfs_path[0]][dir]:
                dir_list.append(dir) 
...
def swap_direction(direction):
    ...
    print(f"swap_dir : {swap_dir}")
    return swap_dir


## run traversal algo
while True:
    ## get current room and dirs
    current_room = visited[player.currentRoom.id]

    ## get list of rooms to
    ## traverse
    room_list = []
    ...
    if len(room_list) > 0:
        ...
        ## pop next directions
        travel_dir = room_list[0]

        # swap prev and curr rooms
        prev_room = visited[player.currentRoom.id]
        prev_room_id = player.currentRoom.id

        ## add dir to traversalPath
        ...
        if player.currentRoom.id not in visited:
            for dir in player.currentRoom.getExits():
                set_rooms.update({dir: '?'})
        
            ## add rooms to visited
            visited[player.currentRoom.id] = set_rooms
        
        # complete swap
        prev_room[travel_dir] = player.currentRoom.id

        ## swap dir
        swap_dir = swap_direction(travel_dir)

        visited[player.currentRoom.id][swap_dir] = prev_room_id
    
    else:
        ## utilize bfs to find a room to traverse
        get_path = bfs_maze(player.currentRoom.id)

        print(f"get_path : {get_path}")

        if get_path is not None:
            dirs = pathfinder(get_path)
